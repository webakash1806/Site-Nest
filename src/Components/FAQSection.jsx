    ...
    <div className="max-w-4xl p-6 mx-auto mt-8 bg-[#F5F6F7] rounded-lg  shadow-xl">
      ...
      <div className="grid justify-center grid-cols-2 gap-2 mb-8 md:grid-cols-4">
        {faqData.map((category, index) => (
          <button
            key={index}
            className={`px-4 py-2 font-semibold duration-300 text-white rounded ${selectedCategory === category.category ? 'bg-blue-600' : 'bg-[#040D43] hover:bg-blue-600'}`}
        ...
        {filteredFaqs.faqs.map((faq, faqIndex) => {
          ...
          return (
            <div key={faqIndex} className="mb-4 duration-300 border border-gray-200 rounded-lg">
              <button
                className={`flex items-center justify-between w-full px-4 py-3 text-left focus:outline-none  duration-100 ${isOpen ? 'bg-[#040D43] text-white' : 'bg-blue-100'} sora-400`}
